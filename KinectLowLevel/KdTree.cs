...
namespace KdTree
{
    ...
    public class KdTree<TValue, TField> : ICollection<TValue>
        where TField : struct, IEquatable<TField>, IFormattable
    {
        // Default location getter function.
        private static readonly Func<TValue, Vector<TField>> defaultLocationGetter =
            el => (Vector<TField>)(object)el;

        // Compares values of underlying field of vector space of locations.
        private static readonly IComparer<TField> fieldComparer = Comparer<TField>.Default;

        // Performs arithmetic on values of field type.
        private static readonly IArithmetic<TField> fieldArithmetic = Arithmetic<TField>.Default;
        ...
        public static KdTree<TValue, TField> Construct(int dimensionality, IEnumerable<TValue> elements,
            Func<TValue, Vector<TField>> locationGetter = null)
        {
            if (elements == null)
                throw new ArgumentNullException("elements");

            // Create and initialize kd-tree.
            var tree = new KdTree<TValue, TField>();
        ...
        private Func<TValue, Vector<TField>> locationGetter;
        ...
        public Func<TValue, Vector<TField>> LocationGetter
        {
        ...
        public IEnumerable<TValue> FindInRange(Vector<TField> location, TField range)
        {
            if (location == null)
        ...
        public TValue FindNearestNeighbor(Vector<TField> location)
        {
            if (location == null)
                throw new ArgumentNullException("location");
        ...
        private class ValueLocationComparer : Comparer<TValue>
        {
            ...
            public ValueLocationComparer(Func<TValue, Vector<TField>> locationGetter)
            {
        ...
        private class ValueDistanceComparer : Comparer<TValue>
        {
            ...
            private Vector<TField> location;

            public ValueDistanceComparer(Func<TValue, Vector<TField>> locationGetter, int dimensionality,
                Vector<TField> location)
            {
                Debug.Assert(locationGetter != null);
                this.locationGetter = locationGetter;
                this.dimensionality = dimensionality;
